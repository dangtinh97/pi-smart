...
try:
    while True:
        try:
            ...
            # Kiểm tra tín hiệu lặp (repeat code)
            ...
            try:
                # Giải mã tín hiệu
                code = decoder.decode_bits(pulses)  # Loại bỏ tham số debug
                ...
                if len(pulses) >= 34:  # Header + 32 bit
                    ...
                    if len(bits) == 32:
                        ...
                        if address + address_inv == 255 and command + command_inv == 255:
                            print("Giải mã NEC thành công!")
                        else:
                            print("Lỗi kiểm tra địa chỉ/lệnh")
